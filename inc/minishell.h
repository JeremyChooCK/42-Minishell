...
char		*perform_replacement(const char *orig, const char *rep,
				const char *with, int count);
...
void		handle_variable(char **src, char **dest,
				char *start, size_t var_len);
...
int			expand_variables_and_count_pipes(char **cmd_parts,
				t_list *data, int *numofpipes);
int			process_command_parts(t_list *data,
				char **cmd_parts, int *numofpipes);
char		*concatenate(const char *str1, const char *str2);
char		*find_command_in_path(char *cmd);
char		*str_replace(char *orig, char *rep, char *with);
void		replace_exit_status(char **command, char *exit_status);
void		process_commands(t_list *data, char **envp,
				char **strarr, int numofpipes);
int			check_if_pipe_is_valid(char **strarr, int numofpipes);
int			execute_piped_commands(t_list *data, char **envp, int numofpipes);
int			execute_multiple_commands(t_list *data,
				char **envp, int numofpipes);
...
int			write_user_input_to_file(int tmpfd, const char *delimiter,
				const char *tmpfile, t_list *data);
int			create_and_write_heredoc(char *delimiter,
				const char *tmpfile, t_list *data);
...
void		check_for_redirection_and_close_pipe(t_list *data,
				int type);
...
void		parse_quotes(const char *prompt,
				int *in_single_quote, int *in_double_quote, int i);
int			calculate_new_length(const char *prompt);
void		update_quote_flags(const char *prompt,
				int *in_single_quote, int *in_double_quote, int index);
void		add_spaces(const char *prompt, char *s,
				int *in_single_quote, int *in_double_quote);
